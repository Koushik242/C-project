  	...
  	if(open_files_decode(decInfo) == e_success)
 	{
		 ...
		 if(decode_magic_str_len(decInfo) == e_success)
		 {
		 	...
		 	if(decode_magic_string(decInfo) == e_success)
		 	{
				 ...
				 {
					 printf("Decoding File size is successfull\n");
					 if(decode_file_extn(decInfo) == e_success)
					 {
						 printf("Decoding File Extension  is successfull\n");
						 if(destination_open_file(decInfo) == e_success)
						 {
							 printf("Destination open file is successfull\n");
							  if(decode_file_size(decInfo->fptr_src_image,&decInfo->file_data_size) == e_success)
							 {
								 printf("Decoding File size is successfull\n");
								 if(decode_secret_data(decInfo) == e_success)
								 {
									 printf("Decoding secret Data  is successfull\n");
								 }
								 else
								 {
									 printf("Decoding secret Data is failed\n");
									 return e_failure;
								 }
						 	}
						 	else
						 	{
								 printf("Decode File Size is failed\n");
								 return e_failure;
							 }
						 }
						 else
						 {
							 printf("Destination open file is failed\n");
							 return e_failure;
						 }
					 }
					 else
				 ...
				 {
					 printf("Decoding file extension size is failed\n");
					 return e_failure;
				 }
			 }
			 else
		 ...
		 {
			 printf("Decoding magic string length is failed\n");
			 return e_failure;
		 }
	}
	else
	{
		printf("Open files decoding is failed\n");
		return e_failure;
	}
...
Status decode_file_extn(DecodeInfo *decInfo)
{
	...
	if(!strstr(decInfo->output_fname, "."))
	{
